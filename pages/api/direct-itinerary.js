...
export default async function handler(req, res) {
  ...
  try {
    // Check authentication - extract the token from request cookies
    // This is the crucial part - ensuring we can access the auth token
    const { data: { session }, error: authError } = await supabase.auth.getSession();
    ...
    // Fail early if no session
    if (!session) {
      console.log("No active session found - authentication failed");
      return res.status(401).json({ 
        error: 'Not authenticated',
        message: 'Please log in to generate an itinerary'
      });
    }
    
    // If we get here, user is authenticated - get their data from request
    const data = req.body;
    
    // Simple validation
    if (!data || !data.destination) {
      return res.status(400).json({ error: 'Missing destination information' });
    }
    
    // Create a simple itinerary (for testing)
    const itinerary = generateSimpleItinerary(data);
    
    // Return the itinerary
    return res.status(200).json({ text: itinerary });
...
// Simple itinerary generator function
function generateSimpleItinerary(data) {
  const destination = data.destination;
  const origin = data.origin || "your location";
  const transportMode = data.transportMode || "preferred transportation";
  
  // Build a simple itinerary
  return `
TRIP TO ${destination.toUpperCase()}
===============================

TRIP SUMMARY
-----------
A journey from ${origin} to ${destination} by ${transportMode}.
...
DAY 2: EXPLORATION
----------------
- Visit the top attractions in ${destination}
